...
def prepare_parser():
  ...
  parser.add_argument(
    ...
    choices=['ppca', 'gaussian', 'nn_dist'],
    help='Scoring function for evaluating manifold density (default: %(default)s)')
  parser.add_argument(
    '--retention_ratio', type=float, default=100,
    help='The percentage of data to keep (default: %(default)s)')
...
def get_data_loaders(dataset, data_root=None, augment=False, batch_size=64, 
                     ...
                     embedding='inceptionv3', density_measure='gaussian',
                     retention_ratio=100, **kwargs):
  ...
  if retention_ratio < 100:
    indices_folder = 'instance_selection_indices/'
    if not os.path.exists(indices_folder):
      os.makedirs(indices_folder)

    indices_filepath = '{}_{}_indices_{}_{}_{}.pkl'\
      .format(dataset, image_size, embedding, density_measure, retention_ratio)
    indices_filepath = os.path.join(indices_folder, indices_filepath)  
    train_set = select_instances(dataset=train_set,
                                 retention_ratio=retention_ratio,
                                 embedding=embedding,
                                 density_measure=density_measure,
                                 indices_filepath=indices_filepath,
