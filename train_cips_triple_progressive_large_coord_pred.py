...
def train(args, loader, loader2, loader3, generator, discriminator, g_optim, d_optim, g_ema, g_ema_temp, g_ema_temp2, device, fid_dataset, fid_dataset2, fid_dataset3, n_scales, writer, path):
    ...
    for p1iter in range(args.iter):
        ...
        fake_pred, _, _ = discriminator(fake, key)
    ...
    for p2iter in range(args.iter):
        ...
        loss_dict['p2image_d_main'] = d_loss_main
        loss_dict['p2image_d_aux'] = d_loss_aux
        ...
        g_loss = g_loss_structure + g_loss_main + g_loss_aux
        ...
        loss_dict['p2image_g_aux'] = g_loss_aux
        ...
        imagep2_d_loss_aux_val = loss_reduced['p2image_d_aux'].mean().item()
        imagep2_g_loss_val = loss_reduced['p2image_g'].mean().item()
        imagep2_g_loss_main_val = loss_reduced['p2image_g_main'].mean().item()
        imagep2_g_loss_aux_val = loss_reduced['p2image_g_aux'].mean().item()
    ...
    for idx in pbar:
        ...
        real_pred, real_pred_coord_h, real_pred_coord_w = discriminator(real, key)
        d_loss_main = d_logistic_loss(real_pred, fake_pred)
        d_loss_aux = args.aux_loss2*d_coordinate_l2_loss(coord_h, real_pred_coord_h, coord_w, real_pred_coord_w) + args.aux_loss2*d_coordinate_l2_loss(coord_h, fake_pred_coord_h, coord_w, fake_pred_coord_w)
        d_loss = d_loss_main + d_loss_aux
        ...
        g_loss_main = g_nonsaturating_loss(fake_pred)
        g_loss_aux = args.aux_loss2*d_coordinate_l2_loss(coord_h, fake_pred_coord_h, coord_w, fake_pred_coord_w)
        g_loss = g_loss_structure + g_loss_main + g_loss_aux
...
if __name__ == '__main__':
    ...
    parser.add_argument('--aux_loss2', type=float, default=0.1)
