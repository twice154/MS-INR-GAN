...
def train(args, loader, loader2, loader2_full, loader3, loader3_full, generator, discriminator, discriminator_full, g_optim, d_optim, d_full_optim, g_ema, g_ema_temp, g_ema_temp2, device, fid_dataset, fid_dataset2, fid_dataset3, n_scales, writer, path):
    loader = sample_data(loader)
    loader2 = sample_data(loader2)
    loader2_full = sample_data(loader2_full)
    loader3 = sample_data(loader3)
    loader3_full = sample_data(loader3_full)
    ...
    if args.distributed:
        ...
        d_full_module = discriminator_full.module

    else:
        g_module = generator
        d_module = discriminator
        d_full_module = discriminator_full
    ...
    for p2iter in range(args.iter):
        ...
        # if random.random() > args.full_ratio:
        start = time.time()
        ...
        #     requires_grad(generator, False)
        #     requires_grad(discriminator_full, True)

        #     noise = mixing_noise(int(args.batch/2), args.latent, args.mixing, device)

        #     fake_img, _ = generator(converted, noise)
        #     fake = fake_img if args.img2dis else torch.cat([fake_img, converted], 1)
        #     fake_pred = discriminator_full(fake, key)

        #     real = real_img if args.img2dis else real_stack
        #     real_pred = discriminator_full(real, key)
        #     d_loss = d_logistic_loss(real_pred, fake_pred)
        ...
        #     requires_grad(generator, True)
        #     requires_grad(discriminator_full, False)
        ...
        #     loss_dict['p2image_g'] = g_loss
        #     loss_dict['p2image_g_main'] = g_loss_main
        #     loss_dict['p2image_g_structure'] = 0
    ...
    for idx in pbar:
        ...
        if random.random() > args.full_ratio:
            ...
            noise = mixing_noise(int(args.batch/4), args.latent, args.mixing, device)
...
def ddp_worker(rank, world_size, args):
    ...
    DiscriminatorFull = getattr(model, args.DiscriminatorFull)
    print('DiscriminatorFull', DiscriminatorFull)
    ...
    discriminator_full = DiscriminatorFull(
        size=args.crop, channel_multiplier=args.channel_multiplier, n_scales=n_scales, input_size=args.dis_input_size,
        n_first_layers=args.n_first_layers,
    ).to(device)
    ...
    if args.distributed:
        ...
        discriminator_full = nn.parallel.DistributedDataParallel(
            discriminator_full,
            device_ids=[rank],
            output_device=rank,
            broadcast_buffers=False,
    ...
    d_full_optim = optim.Adam(
        discriminator_full.parameters(),
        lr=args.lr * d_reg_ratio,
        betas=(0 ** d_reg_ratio, 0.99 ** d_reg_ratio),
    )
    ...
    if args.ckpt is not None:
        ...
        d_full_optim.load_state_dict(ckpt['d_full_optim'])
    ...
    dataset2 = MultiScalePatchProgressivePairedDataset(args.path2, transform=transform, resolution=int(args.coords_size/args.patch_multiplier)*2, resolution_bpg=int(args.coords_size/args.patch_multiplier), crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    dataset2_full = MultiScaleDataset(args.path2, transform=transform, resolution=int(args.coords_size/args.patch_multiplier)*2, crop_size=int(args.coords_size/args.patch_multiplier)*2,
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    dataset3 = MultiScalePatchProgressivePairedDataset(args.path3, transform=transform, resolution=args.coords_size, resolution_bpg=int(args.coords_size/args.patch_multiplier)*2, crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    dataset3_full = MultiScaleDataset(args.path3, transform=transform, resolution=args.coords_size, crop_size=args.coords_size,
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    ...
    train(args, loader, loader2, loader2_full, loader3, loader3_full, generator, discriminator, discriminator_full, g_optim, d_optim, d_full_optim, g_ema, g_ema_temp, g_ema_temp2, device, fid_dataset, fid_dataset2, fid_dataset3, n_scales, writer, path)
...
if __name__ == '__main__':
    ...
    # Partially Full params
    parser.add_argument('--full_ratio', type=float, default=0.1)
    parser.add_argument('--DiscriminatorFull', type=str, default='Discriminator')
