...
def g_structure_simsiam_loss(fake_img, structure_img, fake_crop_params, structure_crop_params, contrastive_encoder, contrastive_predictor):
    ...
    for i in range(len(downsampled_fake_pred)):
        feat_q = F.normalize(downsampled_fake_pred[i], dim=1)
        feat_k = F.normalize(cropped_structure_pred[i], dim=1)
        num_patches = feat_q.shape[0] * feat_q.shape[2] * feat_q.shape[3]
        dim = feat_q.shape[1]
        ...
        centropy_loss = centropy(out, torch.zeros(out.size(0), dtype=torch.long,
