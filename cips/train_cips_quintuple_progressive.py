...
def train(args, loader, loader2, loader3, loader4, loader5, generator, discriminator, g_optim, d_optim, g_ema, g_ema_temp, g_ema_temp2, g_ema_temp3, g_ema_temp4, device, fid_dataset, fid_dataset2, fid_dataset3, fid_dataset4, fid_dataset5, n_scales, writer, path):
    loader = sample_data(loader)
    loader2 = sample_data(loader2)
    loader3 = sample_data(loader3)
    loader4 = sample_data(loader4)
    loader5 = sample_data(loader5)
    ...
    for p2iter in range(args.iter):
        ...
        if p2iter % 500 == 0:
            with torch.no_grad():
                g_ema_temp2.eval()
                converted_full = convert_to_coord_format(sample_z.size(0), int(int(args.coords_size/args.patch_multiplier)*(3/2)), int(int(args.coords_size/args.patch_multiplier)*(3/2)), device,
                                                            integer_values=args.coords_integer_values)
                if args.generate_by_one:
                    converted_full = convert_to_coord_format(1, int(int(args.coords_size/args.patch_multiplier)*(3/2)), int(int(args.coords_size/args.patch_multiplier)*(3/2)), device,
                                                                integer_values=args.coords_integer_values)
                    samples = []
                    for sz in sample_z:
        ...
        if p2iter % args.save_checkpoint_frequency == 0:
            if p2iter > 0:
                cur_metrics = calculate_fid_ddp(g_ema_temp2, fid_dataset=fid_dataset2, bs=args.fid_batch, size=int(int(args.coords_size/args.patch_multiplier)*(3/2)),
                                            num_batches=args.fid_samples//args.fid_batch, latent_size=args.latent,
                                            save_dir=args.path_fid, integer_values=args.coords_integer_values)
                if nsml:
    ...
    ################################################## phase3 training
    for p3iter in range(args.iter):
        print("phase 3, iteration", p3iter)
        ...
        loss_dict['p3image_d'] = d_loss
        loss_dict['p3image_d_main'] = d_loss
        loss_dict['p3image_real_score'] = real_pred.mean()
        loss_dict['p3image_fake_score'] = fake_pred.mean()

        discriminator.zero_grad()
        d_loss.backward()
        d_optim.step()

        d_regularize = p3iter % args.d_reg_every == 0
        ...
        loss_dict['p3image_g'] = g_loss
        loss_dict['p3image_g_main'] = g_loss_main
        loss_dict['p3image_g_structure'] = g_loss_structure

        generator.zero_grad()
        g_loss.backward()
        g_optim.step()

        accumulate(g_ema_temp3, g_module, accum)

        loss_reduced = reduce_loss_dict(loss_dict)

        imagep3_d_loss_val = loss_reduced['p3image_d'].mean().item()
        imagep3_d_loss_main_val = loss_reduced['p3image_d_main'].mean().item()
        imagep3_g_loss_val = loss_reduced['p3image_g'].mean().item()
        imagep3_g_loss_main_val = loss_reduced['p3image_g_main'].mean().item()
        imagep3_g_loss_structure_val = loss_reduced['p3image_g_structure'].mean().item()
        imagep3_r1_val = loss_reduced['p3image_r1'].mean().item()
        imagep3_real_score_val = loss_reduced['p3image_real_score'].mean().item()
        imagep3_fake_score_val = loss_reduced['p3image_fake_score'].mean().item()

        if p3iter % 100 == 0:
            if nsml:
                nsml.report(summary=True, step=p3iter,
                # Generator=g_loss_val,
                # G_main=g_loss_main_val,
                # G_aux=g_loss_aux_val,
                # Discriminator=d_loss_val,
                ...
                # Patch1x_RealScore=patch1x_real_score_val,
                # Patch1x_FakeScore=patch1x_fake_score_val,
                P3_Generator=imagep3_g_loss_val,
                P3_G_main=imagep3_g_loss_main_val,
                P3_G_structure=imagep3_g_loss_structure_val,
                P3_Discriminator=imagep3_d_loss_val,
                P3_D_main=imagep3_d_loss_main_val,
                P3_R1=imagep3_r1_val,
                P3_RealScore=imagep3_real_score_val,
                P3_FakeScore=imagep3_fake_score_val)
            # else:
                # writer.add_scalar("Generator", g_loss_val, i)
                # writer.add_scalar("Discriminator", d_loss_val, i)
                # writer.add_scalar("R1", r1_val, i)
        ...
        if p3iter % 500 == 0:
            with torch.no_grad():
                g_ema_temp3.eval()
                converted_full = convert_to_coord_format(sample_z.size(0), int(args.coords_size/args.patch_multiplier)*2, int(args.coords_size/args.patch_multiplier)*2, device,
                                                            integer_values=args.coords_integer_values)
                if args.generate_by_one:
                    converted_full = convert_to_coord_format(1, int(args.coords_size/args.patch_multiplier)*2, int(args.coords_size/args.patch_multiplier)*2, device,
                                                                integer_values=args.coords_integer_values)
                    samples = []
                    for sz in sample_z:
                        sample, _ = g_ema_temp3(converted_full, [sz.unsqueeze(0)])
                        samples.append(sample)
                    sample = torch.cat(samples, 0)
                else:
                    sample, _ = g_ema_temp3(converted_full, [sample_z])

                utils.save_image(
                    sample,
                    os.path.join(path, 'outputs', args.output_dir, 'p3_images', f'{str(p3iter).zfill(6)}.png'),
                    nrow=int(args.n_sample ** 0.5),
                    normalize=True,
                    range=(-1, 1),
                )
                ...
                if p3iter == 0:
                    utils.save_image(
                        ...
                        os.path.join(
                            path,
                            f'outputs/{args.output_dir}/p3_images/fake_patch_{str(key)}_{str(p3iter).zfill(6)}.png'),
                        nrow=int(fake_img.size(0) ** 0.5),
                        normalize=True,
                        range=(-1, 1),
                    )
                    ...
                    utils.save_image(
                        ...
                        os.path.join(
                            path,
                            f'outputs/{args.output_dir}/p3_images/real_patch_{str(key)}_{str(p3iter).zfill(6)}.png'),
                        nrow=int(real_img.size(0) ** 0.5),
                        normalize=True,
                        range=(-1, 1),
                    )
        if p3iter % args.save_checkpoint_frequency == 0:
            if p3iter > 0:
                cur_metrics = calculate_fid_ddp(g_ema_temp3, fid_dataset=fid_dataset3, bs=args.fid_batch, size=int(args.coords_size/args.patch_multiplier)*2,
                                            num_batches=args.fid_samples//args.fid_batch, latent_size=args.latent,
                                            save_dir=args.path_fid, integer_values=args.coords_integer_values)
                if nsml:
                    nsml.report(summary=True, step=p3iter, p3_fid=cur_metrics['frechet_inception_distance'])
                else:
                    writer.add_scalar("p3_fid", cur_metrics['frechet_inception_distance'], p3iter)
                print(p3iter, "p3_fid",  cur_metrics['frechet_inception_distance'])
    ################################################## phase3 training

    ################################################## phase4 training
    for p4iter in range(args.iter):
        print("phase 4, iteration", p4iter)
        data, coord_h, coord_w, fake_crop_params, structure_crop_params = next(loader4)
        key = np.random.randint(n_scales)
        real_stack = data[key].to(device)
        ...
        loss_dict['p4image_d'] = d_loss
        loss_dict['p4image_d_main'] = d_loss
        loss_dict['p4image_real_score'] = real_pred.mean()
        loss_dict['p4image_fake_score'] = fake_pred.mean()

        discriminator.zero_grad()
        d_loss.backward()
        d_optim.step()

        d_regularize = p4iter % args.d_reg_every == 0
        ...
        fake = fake_img if args.img2dis else torch.cat([fake_img, converted], 1)
        g_ema_temp3.eval()
        structure_img, _ = g_ema_temp3(structured, noise)
        fake_pred = discriminator(fake, key)
        g_loss_structure = args.structure_loss3*g_structure_l2_loss(fake_img, structure_img, fake_crop_params, structure_crop_params)
        g_loss_main = g_nonsaturating_loss(fake_pred)
        g_loss = g_loss_structure + g_loss_main
        ...
        loss_dict['p4image_g'] = g_loss
        loss_dict['p4image_g_main'] = g_loss_main
        loss_dict['p4image_g_structure'] = g_loss_structure

        generator.zero_grad()
        g_loss.backward()
        g_optim.step()

        accumulate(g_ema_temp4, g_module, accum)

        loss_reduced = reduce_loss_dict(loss_dict)

        imagep4_d_loss_val = loss_reduced['p4image_d'].mean().item()
        imagep4_d_loss_main_val = loss_reduced['p4image_d_main'].mean().item()
        imagep4_g_loss_val = loss_reduced['p4image_g'].mean().item()
        imagep4_g_loss_main_val = loss_reduced['p4image_g_main'].mean().item()
        imagep4_g_loss_structure_val = loss_reduced['p4image_g_structure'].mean().item()
        imagep4_r1_val = loss_reduced['p4image_r1'].mean().item()
        imagep4_real_score_val = loss_reduced['p4image_real_score'].mean().item()
        imagep4_fake_score_val = loss_reduced['p4image_fake_score'].mean().item()

        if p4iter % 100 == 0:
            if nsml:
                nsml.report(summary=True, step=p4iter,
                # Generator=g_loss_val,
                # G_main=g_loss_main_val,
                # G_aux=g_loss_aux_val,
                # Discriminator=d_loss_val,
                ...
                # Patch1x_RealScore=patch1x_real_score_val,
                # Patch1x_FakeScore=patch1x_fake_score_val,
                P4_Generator=imagep4_g_loss_val,
                P4_G_main=imagep4_g_loss_main_val,
                P4_G_structure=imagep4_g_loss_structure_val,
                P4_Discriminator=imagep4_d_loss_val,
                P4_D_main=imagep4_d_loss_main_val,
                P4_R1=imagep4_r1_val,
                P4_RealScore=imagep4_real_score_val,
                P4_FakeScore=imagep4_fake_score_val)
            # else:
                # writer.add_scalar("Generator", g_loss_val, i)
                # writer.add_scalar("Discriminator", d_loss_val, i)
                # writer.add_scalar("R1", r1_val, i)
        ...
        if p4iter % 500 == 0:
            with torch.no_grad():
                g_ema_temp4.eval()
                converted_full = convert_to_coord_format(sample_z.size(0), int(int(args.coords_size/args.patch_multiplier)*2*(3/2)), int(int(args.coords_size/args.patch_multiplier)*2*(3/2)), device,
                                                            integer_values=args.coords_integer_values)
                if args.generate_by_one:
                    converted_full = convert_to_coord_format(1, int(int(args.coords_size/args.patch_multiplier)*2*(3/2)), int(int(args.coords_size/args.patch_multiplier)*2*(3/2)), device,
                                                                integer_values=args.coords_integer_values)
                    samples = []
                    for sz in sample_z:
                        sample, _ = g_ema_temp4(converted_full, [sz.unsqueeze(0)])
                        samples.append(sample)
                    sample = torch.cat(samples, 0)
                else:
                    sample, _ = g_ema_temp4(converted_full, [sample_z])

                utils.save_image(
                    sample,
                    os.path.join(path, 'outputs', args.output_dir, 'p4_images', f'{str(p4iter).zfill(6)}.png'),
                    nrow=int(args.n_sample ** 0.5),
                    normalize=True,
                    range=(-1, 1),
                )
                ...
                if p4iter == 0:
                    utils.save_image(
                        ...
                        os.path.join(
                            path,
                            f'outputs/{args.output_dir}/p4_images/fake_patch_{str(key)}_{str(p4iter).zfill(6)}.png'),
                        nrow=int(fake_img.size(0) ** 0.5),
                        normalize=True,
                        range=(-1, 1),
                    )
                    ...
                    utils.save_image(
                        ...
                        os.path.join(
                            path,
                            f'outputs/{args.output_dir}/p4_images/real_patch_{str(key)}_{str(p4iter).zfill(6)}.png'),
                        nrow=int(real_img.size(0) ** 0.5),
                        normalize=True,
                        range=(-1, 1),
                    )
        if p4iter % args.save_checkpoint_frequency == 0:
            if p4iter > 0:
                cur_metrics = calculate_fid_ddp(g_ema_temp4, fid_dataset=fid_dataset4, bs=args.fid_batch, size=int(int(args.coords_size/args.patch_multiplier)*2*(3/2)),
                                            num_batches=args.fid_samples//args.fid_batch, latent_size=args.latent,
                                            save_dir=args.path_fid, integer_values=args.coords_integer_values)
                if nsml:
                    nsml.report(summary=True, step=p4iter, p4_fid=cur_metrics['frechet_inception_distance'])
                else:
                    writer.add_scalar("p4_fid", cur_metrics['frechet_inception_distance'], p4iter)
                print(p4iter, "p4_fid",  cur_metrics['frechet_inception_distance'])
    ################################################## phase4 training
    ...
    for idx in pbar:
        ...
        data, coord_h, coord_w, fake_crop_params, structure_crop_params = next(loader5)
        key = np.random.randint(n_scales)
        real_stack = data[key].to(device)
        ...
        fake = fake_img if args.img2dis else torch.cat([fake_img, converted], 1)
        g_ema_temp4.eval()
        structure_img, _ = g_ema_temp4(structured, noise)
        fake_pred = discriminator(fake, key)
        g_loss_structure = args.structure_loss4*g_structure_l2_loss(fake_img, structure_img, fake_crop_params, structure_crop_params)
        g_loss_main = g_nonsaturating_loss(fake_pred)
        g_loss = g_loss_structure + g_loss_main
        ...
        if get_rank() == 0:
            ...
            if i % args.save_checkpoint_frequency == 0:
                torch.save(
                    {
                        ...
                        'g_ema_temp3': g_ema_temp3.state_dict(),
                        'g_ema_temp4': g_ema_temp4.state_dict(),
                ...
                if i > 0:
                    cur_metrics = calculate_fid_ddp(g_ema, fid_dataset=fid_dataset5, bs=args.fid_batch, size=args.coords_size,
                                                num_batches=args.fid_samples//args.fid_batch, latent_size=args.latent,
                                                save_dir=args.path_fid, integer_values=args.coords_integer_values)
                    if nsml:
...
def ddp_worker(rank, world_size, args):
    ...
    os.makedirs(os.path.join(path, 'outputs', args.output_dir, 'p3_images'), exist_ok=True)
    os.makedirs(os.path.join(path, 'outputs', args.output_dir, 'p4_images'), exist_ok=True)
    ...
    g_ema_temp3 = Generator(size=int(args.size/args.patch_multiplier), hidden_size=args.fc_dim, style_dim=args.latent, n_mlp=args.n_mlp,
                    activation=args.activation, channel_multiplier=args.channel_multiplier,
                    ).to(device)
    g_ema_temp3.eval()
    accumulate(g_ema_temp3, generator, 0)
    g_ema_temp4 = Generator(size=int(args.size/args.patch_multiplier), hidden_size=args.fc_dim, style_dim=args.latent, n_mlp=args.n_mlp,
                    activation=args.activation, channel_multiplier=args.channel_multiplier,
                    ).to(device)
    g_ema_temp4.eval()
    accumulate(g_ema_temp4, generator, 0)
    ...
    if args.ckpt is not None:
        ...
        g_ema_temp3.load_state_dict(ckpt['g_ema_temp3'])
        g_ema_temp4.load_state_dict(ckpt['g_ema_temp4'])
    ...
    if nsml:
        ...
        args.path4 = os.path.join(DATASET_PATH, 'train', args.path4.split('/')[-1])
        args.path5 = os.path.join(DATASET_PATH, 'train', args.path5.split('/')[-1])
    else:
        pass
    ...
    dataset2 = MultiScalePatchProgressivePairedDataset(args.path2, transform=transform, resolution=int(int(args.coords_size/args.patch_multiplier)*(3/2)), resolution_bpg=int(args.coords_size/args.patch_multiplier), crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    dataset3 = MultiScalePatchProgressivePairedDataset(args.path3, transform=transform, resolution=int(args.coords_size/args.patch_multiplier)*2, resolution_bpg=int(int(args.coords_size/args.patch_multiplier)*(3/2)), crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    dataset4 = MultiScalePatchProgressivePairedDataset(args.path4, transform=transform, resolution=int(int(args.coords_size/args.patch_multiplier)*2*(3/2)), resolution_bpg=int(args.coords_size/args.patch_multiplier)*2, crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    dataset5 = MultiScalePatchProgressivePairedDataset(args.path5, transform=transform, resolution=args.coords_size, resolution_bpg=int(int(args.coords_size/args.patch_multiplier)*2*(3/2)), crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    ...
    fid_dataset2 = ImageDataset(args.path2, transform=transform_fid, resolution=int(int(args.coords_size/args.patch_multiplier)*(3/2)), to_crop=args.to_crop)
    fid_dataset2.length = args.fid_samples
    fid_dataset3 = ImageDataset(args.path3, transform=transform_fid, resolution=int(args.coords_size/args.patch_multiplier)*2, to_crop=args.to_crop)
    fid_dataset3.length = args.fid_samples
    fid_dataset4 = ImageDataset(args.path4, transform=transform_fid, resolution=int(int(args.coords_size/args.patch_multiplier)*2*(3/2)), to_crop=args.to_crop)
    fid_dataset4.length = args.fid_samples
    fid_dataset5 = ImageDataset(args.path5, transform=transform_fid, resolution=args.coords_size, to_crop=args.to_crop)
    fid_dataset5.length = args.fid_samples
    ...
    train(args, loader, loader2, loader3, loader4, loader5, generator, discriminator, g_optim, d_optim, g_ema, g_ema_temp, g_ema_temp2, g_ema_temp3, g_ema_temp4, device, fid_dataset, fid_dataset2, fid_dataset3, fid_dataset4, fid_dataset5, n_scales, writer, path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('path5', type=str)
    ...
    parser.add_argument('--structure_loss3', type=float, default=0.1)
    parser.add_argument('--structure_loss4', type=float, default=0.1)
