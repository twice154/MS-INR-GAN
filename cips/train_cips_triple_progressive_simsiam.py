...
def g_structure_simsiam_loss(fake_img, structure_img, fake_crop_params, structure_crop_params, contrastive_encoder, contrastive_predictor):
    ...
    for i in range(len(downsampled_fake_enc)):
        loss1 = cosine(downsampled_fake_pred[i], cropped_structure_enc[i].clone().detach()).mean()
        loss2 = cosine(downsampled_fake_enc[i].clone().detach(), cropped_structure_pred[i]).mean()
        loss += (loss1 + loss2)
