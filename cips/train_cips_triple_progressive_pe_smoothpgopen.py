...
def train(args, loader, loader2, loader3, generator, discriminator, g_optim, d_optim, g_ema, g_ema_temp, g_ema_temp2, device, fid_dataset, fid_dataset2, fid_dataset3, n_scales, writer, path):
    ...
    g_module.update_pe_activation_mask(unmasking_ratio=args.unmasking_ratio1, device=device)
    g_ema_temp.update_pe_activation_mask(unmasking_ratio=args.unmasking_ratio1, device=device)
    ...
    g_module.update_pe_activation_mask(unmasking_ratio=args.unmasking_ratio1, device=device)
    g_ema_temp2.update_pe_activation_mask(unmasking_ratio=args.unmasking_ratio1, device=device)

    for p2iter in range(args.iter):
        print("phase 2, iteration", p2iter)

        g_module.progressive_and_smooth_open_pe_activation_mask(unmasking_ratio=args.unmasking_ratio2, previous_unmasking_ratio=args.unmasking_ratio1, total_iteration=args.pgopen_iter, current_iteration=p2iter, device=device)
        g_ema_temp2.progressive_and_smooth_open_pe_activation_mask(unmasking_ratio=args.unmasking_ratio2, previous_unmasking_ratio=args.unmasking_ratio1, total_iteration=args.pgopen_iter, current_iteration=p2iter, device=device)
    ...
    g_module.update_pe_activation_mask(unmasking_ratio=args.unmasking_ratio2, device=device)
    g_ema.update_pe_activation_mask(unmasking_ratio=args.unmasking_ratio1, device=device)

    for idx in pbar:
        i = idx + args.start_iter

        g_module.progressive_and_smooth_open_pe_activation_mask(unmasking_ratio=args.unmasking_ratio3, previous_unmasking_ratio=args.unmasking_ratio2, total_iteration=args.pgopen_iter, current_iteration=i, device=device)
        g_module.progressive_and_smooth_open_pe_activation_mask(unmasking_ratio=args.unmasking_ratio3, previous_unmasking_ratio=args.unmasking_ratio2, total_iteration=args.pgopen_iter, current_iteration=i, device=device)
