...
import model
from dataset import MultiScaleDataset, ImageDataset, MultiScalePatchDataset, MultiScaleScaleDataset, MultiScalePatchScaleDataset
from calculate_fid import calculate_fid, calculate_fid_scale
...
def train(args, loader, coarse_loader, generator, discriminator, coarse_discriminator, g_optim, d_optim, coarse_d_optim, g_ema, device, fid_dataset):
    ...
    for idx in pbar:
        ...
        d_regularize = i % args.coarse_d_reg_every == 0
        ...
        if get_rank() == 0:
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    hw_full = torch.tensor(1 / args.size).reshape(1, 1, 1, 1)
                    sq_full = torch.tensor(1 / (args.size * args.size)).reshape(1, 1, 1, 1)
                    scales_full = torch.cat((hw_full, sq_full), 1).repeat(sample_z.size(0), 1, args.size, args.size).to(device)
                    converted_full = torch.cat((converted_full, scales_full), 1)
                    if args.generate_by_one:
                        converted_full = convert_to_coord_format(1, args.size, args.size, device,
                                                                 integer_values=args.coords_integer_values)
                        hw_full = torch.tensor(1 / args.size).reshape(1, 1, 1, 1)
                        sq_full = torch.tensor(1 / (args.size * args.size)).reshape(1, 1, 1, 1)
                        scales_full = torch.cat((hw_full, sq_full), 1).repeat(1, 1, args.size, args.size).to(device)
                        converted_full = torch.cat((converted_full, scales_full), 1)
                        samples = []
                        for sz in sample_z:
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    converted_full = convert_to_coord_format(sample_z.size(0), int(args.size/4), int(args.size/4), device,
                                                             integer_values=args.coords_integer_values)
                    hw_full = torch.tensor(1 / int(args.size/4)).reshape(1, 1, 1, 1)
                    sq_full = torch.tensor(1 / (int(args.size/4) * int(args.size/4))).reshape(1, 1, 1, 1)
                    scales_full = torch.cat((hw_full, sq_full), 1).repeat(sample_z.size(0), 1, int(args.size/4), int(args.size/4)).to(device)
                    converted_full = torch.cat((converted_full, scales_full), 1)
                    if args.generate_by_one:
                        ...
                        hw_full = torch.tensor(1 / int(args.size/4)).reshape(1, 1, 1, 1)
                        sq_full = torch.tensor(1 / (int(args.size/4) * int(args.size/4))).reshape(1, 1, 1, 1)
                        scales_full = torch.cat((hw_full, sq_full), 1).repeat(1, 1, int(args.size/4), int(args.size/4)).to(device)
                        converted_full = torch.cat((converted_full, scales_full), 1)
                        samples = []
                        for sz in sample_z:
                            sample, _ = g_ema(converted_full, [sz.unsqueeze(0)])
            ...
            if i % args.save_checkpoint_frequency == 0:
                ...
                if i > 0:
                    cur_metrics = calculate_fid_scale(g_ema, fid_dataset=fid_dataset, bs=args.fid_batch, size=args.coords_size,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale_input_fourier', type=bool, default=True)
    parser.add_argument('--scale_input_fourier_dim', type=int, default=512)
    ...
    generator = Generator(size=args.size, hidden_size=args.fc_dim, style_dim=args.latent, n_mlp=args.n_mlp,
                          ...
                          scale_input=args.scale_input, scale_input_fourier=args.scale_input_fourier, scale_input_fourier_dim=args.scale_input_fourier_dim
                          ).to(device)
    ...
    if nsml:
        ...
        pass
    dataset = MultiScalePatchScaleDataset(args.path, transform=transform, resolution=args.coords_size, crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    coarse_dataset = MultiScaleScaleDataset(args.coarse_path, transform=transform, resolution=int(args.coords_size/args.patch_multiplier), crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
