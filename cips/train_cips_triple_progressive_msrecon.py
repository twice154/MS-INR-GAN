...
import model
from dataset import MultiScaleDataset, ImageDataset, MultiScalePatchDataset, MultiScalePatchProgressiveDataset, MultiScalePatchProgressivePairedDataset, MultiScalePatchProgressiveTripledDataset
...
def train(args, loader, loader2, loader3, generator, discriminator, g_optim, d_optim, g_ema, g_ema_temp, g_ema_temp2, device, fid_dataset, fid_dataset2, fid_dataset3, n_scales, writer, path):
    ...
    for idx in pbar:
        ...
        data, coord_h, coord_w, fake_crop_params, structure_crop_params, structure_crop_params_triple = next(loader3)
        key = np.random.randint(n_scales)
        real_stack = data[key].to(device)

        real_img, converted, structured, structured_triple = real_stack[:, :3], real_stack[:, 3:5], real_stack[:, 5:7], real_stack[:, 7:9]
        ...
        g_ema_temp.eval()
        structure_img_triple, _ = g_ema_temp(structured_triple, noise)
        fake_pred = discriminator(fake, key)
        g_loss_structure = args.structure_loss2*g_structure_l2_loss(fake_img, structure_img, fake_crop_params, structure_crop_params) + args.structure_loss2_triple*g_structure_l2_loss(fake_img, structure_img_triple, fake_crop_params, structure_crop_params_triple)
        g_loss_main = g_nonsaturating_loss(fake_pred)
        g_loss = g_loss_structure + g_loss_main
...
def ddp_worker(rank, world_size, args):
    ...
    dataset2 = MultiScalePatchProgressivePairedDataset(args.path2, transform=transform, resolution=int(args.coords_size/args.patch_multiplier)*2, resolution_bpg=int(args.coords_size/args.patch_multiplier), crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    dataset3 = MultiScalePatchProgressiveTripledDataset(args.path3, transform=transform, resolution=args.coords_size, resolution_bpg=int(args.coords_size/args.patch_multiplier)*2, resolution_bbpg=int(args.coords_size/args.patch_multiplier), crop_size=int(args.crop/args.patch_multiplier),
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--structure_loss2_triple', type=float, default=0.1)
