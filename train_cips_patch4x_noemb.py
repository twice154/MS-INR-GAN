...
import model
from dataset_noemb import MultiScaleDataset, ImageDataset
    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            ...
            if i % args.save_checkpoint_frequency == 0:
                ...
                if i > 0:
                    cur_metrics = calculate_fid(g_ema, fid_dataset=fid_dataset, bs=args.fid_batch, size=args.coords_size*4,
...
if __name__ == '__main__':
    ...
    if nsml:
        args.train_path = os.path.join(DATASET_PATH, 'train', args.train_path.split('/')[-1])
        args.test_path = os.path.join(DATASET_PATH, 'train', args.test_path.split('/')[-1])
    else:
        pass
    dataset = MultiScaleDataset(args.train_path, transform=transform, resolution=args.coords_size, crop_size=args.crop,
                                integer_values=args.coords_integer_values, to_crop=args.to_crop)
    fid_dataset = ImageDataset(args.test_path, transform=transform_fid, resolution=args.coords_size, to_crop=args.to_crop)
