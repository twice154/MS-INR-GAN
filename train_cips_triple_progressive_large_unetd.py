...
def train(args, loader, loader2, loader3, generator, discriminator, g_optim, d_optim, g_ema, g_ema_temp, g_ema_temp2, device, fid_dataset, fid_dataset2, fid_dataset3, n_scales, writer, path):
    ...
    for p1iter in range(args.iter):
        ...
        fake_pred, fake_pred_pixel = discriminator(fake, key)

        real = real_img if args.img2dis else real_stack
        real_pred, real_pred_pixel = discriminator(real, key)
        d_loss = d_logistic_loss(real_pred, fake_pred)
        d_loss_pixel = d_logistic_loss(real_pred_pixel, fake_pred_pixel)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unetd_global_loss', type=float, default=1)
    parser.add_argument('--unetd_pixel_loss', type=float, default=1)
